...
对时间段内的隐患 首先经过tf-idf进行关键词提取，每个主题保留五个关键词  再word2vec,然后进行dbscan聚类，结果存在word_closter2.xlsx中
...
def train_model(texts):
    ...
    model.save("mymodel/word2vec+dbscan.bin")
    return model


def word_cluster(texts,model):
    # 分词
    ...
    labels = DBSCAN(eps=0.02,min_samples=50).fit_predict(key_words_vectors)
    # 输出excel
    ...
    df.to_excel('word2vec+dbscan.xls', index=False)
...
if __name__ == "__main__":
    ...
    # 进行聚类，包括了分词，word2vec，dbscan

    word_cluster(time_window,model)
